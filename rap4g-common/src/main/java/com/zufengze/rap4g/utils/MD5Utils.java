package com.zufengze.rap4g.utils;
...
public class MD5Utils {
    ...
    public static final String NAME_FORMAT_WARN_MSG = "名字必须由数字/字母/汉子/空格/下划线组成,长度" + NAME_LENGTH_MIN + "-" + NAME_LENGTH_MAX + ".";
    public static final String ACCOUNT_FORMAT_WARN_MSG = "账户必须由数字/字母/下划线组成,长度" + ACCOUNT_LENGTH_MIN + "-" + ACCOUNT_LENGTH_MAX + ".";
    /**
     * 默认编码 utf8
     */
    ...
    public static String removeIllegalCharacters(String o) {
        return o.replaceAll(ILLEGAL_NAME_CHAR, "");
    }

    public static boolean validateAccount(String str) {
        if (str == null) return false;
        if (str.length() < ACCOUNT_LENGTH_MIN || str.length() > ACCOUNT_LENGTH_MAX) {
            return false;
        }
        return str.matches(LEGAL_ACCOUNT_CHAR + "*");
    }

    public static boolean validateName(String str) {
        if (str == null) return false;
        if (str.length() < NAME_LENGTH_MIN || str.length() > NAME_LENGTH_MAX) {
            return false;
        }

        for (int i = 0; i < str.length(); i++) {
            char c = str.charAt(i);
            if (!isChinese(c) && !String.valueOf(c).matches(LEGAL_NAME_CHAR)) {
                return false;
            }
        }
